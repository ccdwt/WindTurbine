...
#
set daemon  20              # check services at 20 second intervals
with start delay 0
...
#
 set logfile /var/log/monit.log #syslog facility log_daemon                       
#
#
...
set mailserver smtp.gmail.com port 587
    username "calvinwindenergy@gmail.com" password "windenergy"
    using tlsv1
...
#
 set mail-format { from: calvinWindEnergy@gmail.com }
#
#
...
check process turbine_log with pidfile `'PIDDIR`'/turbine_log.pid
  ...
  if changed pid 5 times within 5 cycles then alert
...
check file weather_logfile with path `'LOGDIR`'/weather_log.csv
  ...
  stop program = "/etc/init.d/weather stop"
  if timestamp > 90 seconds then restart
  if timestamp > 1 hour then alert #prm5@students.calvin.edu
