...
APP_KEY = 'pg85fmq2slj3p7j'
APP_SECRET = 'ex3whqzzabg6w75'
...
class DropboxTerm(cmd.Cmd):
    ...
    def do_put(self, from_path, to_path):
        ...
        self.api_client.put_file(self.current_path + "/" + to_path, from_file,True)
...
#   term = DropboxTerm(APP_KEY, APP_SECRET)
    api_client = None
    try:
        token = open("/usr/src/WindTurbine/utilities/dropbox/token_store.txt").read()
        api_client = client.DropboxClient(token)
        print "[loaded access token]"
    except IOError:
        pass # don't worry if it's not there

    from_file = open(os.path.expanduser("/usr/src/WindTurbine/db/test.db"), "rb")
